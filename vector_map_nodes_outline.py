...
gridsize = 20
cellplotsize = 55-1.5*(gridsize-10)
...
def draw_line_and_intersection(node, node2, route):
    ...
    if intersection:
        draw_target_agent_line(node, intersection)
        (intersection_x, intersection_y) = intersection.coordinate
        ax.plot(intersection_x, intersection_y, "go", markersize=10)
        plt.draw()
        outline_route(node, node2, intersection, route)
    else:
        draw_target_agent_line(node, node2)
...
def find_outline_segment(start_node, target_node, node, route):
    ...
    if point_to_segment_distance(node.coordinate, agent.coordinate, target.coordinate)[1] <= 1:
    ...
    agent_target_first_last_intersection_mid = first_last_intersection_midpoint(agent, target)
    ax.plot(agent_target_first_last_intersection_mid[0], agent_target_first_last_intersection_mid[1], "yo", markersize=10)
    first_intersection = find_agent_target_coordinate_intersection(agent, target)
    last_intersection = find_agent_target_coordinate_intersection(target, agent)
    for vn in node.vectorNodes:
        ...
        # (ptA, start_node_to_segment) = point_to_segment_distance(start_node.coordinate, node.coordinate, vn.coordinate)
        # (ptT, target_node_to_segment) = point_to_segment_distance(target_node.coordinate, node.coordinate, vn.coordinate)
        agent_to_hpt = distance.euclidean(agent.coordinate, halfway_pt)
        agent_to_ints = distance.euclidean(agent.coordinate, first_intersection)
        target_to_hpt = distance.euclidean(target.coordinate, halfway_pt)
        target_to_ints = distance.euclidean(target.coordinate, last_intersection)
        fst_to_hpt = distance.euclidean(first_intersection, halfway_pt)
        lst_to_hpt = distance.euclidean(last_intersection, halfway_pt)
        mid_to_segment = distance.euclidean(agent_target_first_last_intersection_mid, halfway_pt)
        ints_to_mid = distance.euclidean(agent_target_first_last_intersection_mid, first_intersection)
        mid_to_segment_normalized = mid_to_segment/ints_to_mid
        fst_to_hpt_normalized = fst_to_hpt/ints_to_mid if fst_to_hpt < 3 else 0
        lst_to_hpt_normalized = lst_to_hpt/ints_to_mid if fst_to_hpt == 0 and  lst_to_hpt < 3 else 0
        agent_to_hpt_normalized = agent_to_hpt/agent_to_ints # if fst_to_hpt < 3 else 0
        target_to_hpt_normalized = target_to_hpt/target_to_ints  #if agent_to_hpt_normalized == 0 and lst_to_hpt < 3 else 0
        #choose s so that we will select the route along the outer edge, but making sure it is the shortest route to the target
        #mid_to_segment_normalized = mid_to_segment_normalized if agent_to_hpt_normalized == 0 and target_to_hpt_normalized == 0 else 0
        factor = agent_to_hpt_normalized if agent_to_hpt_normalized < target_to_hpt_normalized else -target_to_hpt_normalized
        # s = mid_to_segment_normalized + factor
        s = 0.5*mid_to_segment_normalized + agent_to_hpt_normalized - 3*target_to_hpt_normalized
        if vn not in route:
...
def add_cell(coordinate):
    ...
    ax.plot(cell.coordinate[0], cell.coordinate[1], "rs", markersize=cellplotsize)
