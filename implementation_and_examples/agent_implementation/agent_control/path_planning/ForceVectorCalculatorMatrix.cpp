...
//                                                                              this->ticks_per_second);
    const std::vector<std::pair<int, int>> frontiers = getFrontierCells(agent, agent->elapsed_ticks / agent->ticks_per_second, agent->config.FRONTIER_SEARCH_RADIUS);
...
//    std::vector<std::vector<Coordinate>> frontierRegions = {};
    ...
    for (const auto& cell : frontiers) {
        if (!visited.count(cell)) {
            ...
            std::vector<std::pair<int, int>> group = frontierMerger.bfs_group(cell.first, cell.second, agent->coverageMatrix->getWidth(), agent->coverageMatrix->getHeight(), frontierset, visited);
    ...
    for (const auto &region: frontierRegions) {
        ...
        for (auto cellIndex: region) {
            auto realCellCoordinate = agent->coverageMatrix->getRealCoordinateFromIndex(cellIndex.first, cellIndex.second);
...
std::vector<std::pair<int, int>> ForceVectorCalculator::getFrontierCells(Agent* agent, double currentTimeS, double searchRadius) {
    ...
    auto i_min = std::max(0, agent->coverageMatrix->getIndexFromRealCoordinate({agent->position.x - searchRadius, 0}).first);
    auto i_max = std::min(agent->coverageMatrix->getWidth(), agent->coverageMatrix->getIndexFromRealCoordinate({agent->position.x + searchRadius, 0}).first);
    auto j_min = std::max(0, agent->coverageMatrix->getIndexFromRealCoordinate({0, agent->position.y - searchRadius}).second);
    auto j_max = std::min(agent->coverageMatrix->getHeight(), agent->coverageMatrix->getIndexFromRealCoordinate({0, agent->position.y + searchRadius}).second);


    for (int i = i_min; i < i_max; i++) {
        for (int j = j_min; j < j_max; j++) {
            if(agent->coverageMatrix->getByIndex(i, j, currentTimeS) > 0){ //Check if the cell is explored
                Coordinate realCoordinate = agent->coverageMatrix->getRealCoordinateFromIndex(i, j);
                auto [i_obstacle, j_obstacle] = agent->obstacleMatrix->getIndexFromRealCoordinate(realCoordinate);
                //Check if at least one neighbor is unexplored

                std::array<double, 9> coverageMatrixNeighbors = agent->coverageMatrix->MooreNeighbors(i, j, currentTimeS);
                std::array<double, 9> obstacleMatrixNeighbors = agent->obstacleMatrix->MooreNeighbors(i_obstacle, j_obstacle, currentTimeS);
