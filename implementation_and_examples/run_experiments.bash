...
for r in $(seq 1 $((N_REPEATED_EXPERIMENTS))); do
  ...
  #        export CONFIG_FILE
          ...
          sed "s|{{CONFIG_PATH}}|${CONFIG_PATH}|g" "$EXP_PATH" > "temp_${CONFIG_FILE%.yaml}_${EXPERIMENT}"
          ...
          for i in $(seq 1 $((N_AGENTS))); do
            ...
            sed -i "s/{{${AGENT_ID}_ORIENTATION}}/${ORIENTATION}/" "temp_${CONFIG_FILE%.yaml}_${EXPERIMENT}"
            ...
            REMAINING_AGENTS=$((N_AGENTS-remove_agents))
              ...
              if [ $remove_agents -ne 0 ]; then
                  for i in $(seq 1 $((remove_agents))); do
                    sed -i "/<pipuck id=\"pipuck$((N_AGENTS-i+1))\"/,/<\/pipuck>/d" "temp_${CONFIG_FILE%.yaml}_${EXPERIMENT}"
                  done
              fi
  ...
  #            for AVERAGE_INTER_SPAWN_TIME in {120..20..0} # for loop from 300 to 30 with step 30
              ...
              do
                ...
                sed "s|{{FRAME_PATH}}|${FRAME_PATH}|g" "temp_${CONFIG_FILE%.yaml}_${EXPERIMENT}" > "temp_${CONFIG_FILE%.yaml}_${REMAINING_AGENTS}_${AVERAGE_INTER_SPAWN_TIME}_${EXPERIMENT}"
  ...
  #              LOGFILE="$LOG_DIR/${EXPERIMENT%.argos}_${CONFIG_FILE%.yaml}_spawn_time_${AVERAGE_INTER_SPAWN_TIME}_${REMAINING_AGENTS}_agents_$TIMESTAMP.log"
                ...
                "$ARGOSEXEC" -n -c "temp_${CONFIG_FILE%.yaml}_${REMAINING_AGENTS}_${AVERAGE_INTER_SPAWN_TIME}_${EXPERIMENT}" > "$LOGFILE" 2>&1 &
                PID=$!
                echo "ARGoS3 process $PID started"
                pids[$PID]="$EXPERIMENT|$CONFIG_FILE|$AVERAGE_INTER_SPAWN_TIME|$REMAINING_AGENTS|$LOGFILE"

                #wait 5 seconds before starting the next experiment, so it has time to initialize the exported variables
                sleep 5
              done
        done
...
for pid in "${!pids[@]}"; do
    ...
    IFS='|' read -r EXPERIMENT CONFIG_FILE AVERAGE_INTER_SPAWN_TIME REMAINING_AGENTS LOGFILE <<< "${pids[$pid]}"
