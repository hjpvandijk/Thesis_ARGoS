...
def process_row(row, coverage_matrix, resolution, actual_arena_polygon, actual_arena2_polygon, outside_area, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, plot, ax):
    ...
    if coverage_matrix:
        pheromone = float(row['coverage_pheromone'])
    else:
        pheromone = float(row['obstacle_pheromone'])
    if pheromone == 0:
        return []
...
def parallel_process(coverage_matrix_data, obstacle_matrix_data, resolution, actual_arena_polygon, actual_arena2_polygon, outside_area, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, plot, ax):
    true_positives = 0
    false_positives = 0
    false_negatives = 0
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures_coverage = [executor.submit(process_row, row, True, resolution, actual_arena_polygon, actual_arena2_polygon, outside_area, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, plot, ax) for row in coverage_matrix_data]
        futures_obstacle = [executor.submit(process_row, row, False, resolution, actual_arena_polygon, actual_arena2_polygon, outside_area, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, plot, ax) for row in obstacle_matrix_data]

        obstacles_minus_1 = []
        
        
        # # Collect the results as they are completed
        for future in concurrent.futures.as_completed(futures_obstacle):
            ...
            for res in results:
                ...
                if box_size == -1:
                    ...
                    continue
                if pheromone == -1:
                    obstacles_minus_1.append((box_x, box_y))
                # Determine results based on pheromone and overlap status
                if box_contains_arena and (pheromone > 0 or pheromone == -1):
                    true_positives += 1 # Correctly identified as occupied
                    color = 'red'
                # if box_contains_arena and pheromone > 0:
                #     false_negatives += 1 # Incorrectly identified as free (actually occupied)
                #     color = 'pink'
                # elif not box_contains_arena and pheromone > 0:
                #     true_positives += 1 # Correctly identified as free
                #     color = 'green'
                elif not box_contains_arena and (pheromone > 0 or pheromone == -1):
         ...
         # # Collect the results as they are completed
        for future in concurrent.futures.as_completed(futures_coverage):
            results = future.result()
            # You can now process the results, e.g., accumulate them or update shared data
            # This could involve updating `true_positives`, `false_negatives`, etc.
            for res in results:
                ...
                if box_size == -1:
                    ...
                    continue
                if pheromone == -1:
                    if (box_x, box_y) in obstacles_minus_1:
                        # print("Found a box that is both an obstacle and a coverage pheromone")
                        continue # This box is already counted as an obstacle
                # Determine results based on pheromone and overlap status
...
def calculate_precision_recall_coverage(union_map, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, coverage_data, obstacle_data, spawn_time, plot):
    ...
    true_positives, false_positives, false_negatives, n_cell_outside_area = parallel_process(coverage_data, obstacle_data, resolution, actual_arena_polygon, actual_arena2_polygon, outside_area, str_tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, plot, ax)

    covered_cels = true_positives + false_positives + false_negatives
    coverage = covered_cels * resolution * resolution / get_total_area_from_map_spawn_time(env_map, spawn_time)
...
# coverage_data_file = 'scripts_metric_plots/coverage_matrix_map_relayed_pipuck15.csv'
# obstacle_data_file = 'scripts_metric_plots/obstacle_matrix_map_relayed_pipuck15.csv'
...
# coverage_data = read_matrix_data(coverage_data_file)
# obstacle_data = read_matrix_data(obstacle_data_file)
...
# precision, recall = calculate_precision_recall(u_map,gridtree, grid, coverage_data, obstacle_data, spawn_time, True)
