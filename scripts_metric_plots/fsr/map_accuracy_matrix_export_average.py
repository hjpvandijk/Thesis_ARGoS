...
from map_accuracy_matrix import set_variables_based_on_map, read_arena_boxes, read_arena_spawn_boxes, create_arena_rectangles_and_circles, read_arena_cylinders, union_map, union_u_map_and_spawn_boxes, divide_map_into_grid_tree, calculate_precision_recall_coverage, get_spawn_times
...
def read_matrix_data(zf, filename):
                                        ...
                                        for spawn_time in spawn_times:
                                            for seed in seeds:
                                                config = create_experiment_string(
                                                    end_time, noise, comm_range, message_loss_probability,
                                                    frontier_search_radius, evaporation_time
                                                )
...
def process_experiment(seed, map, prefix, config, spawn_time, agent, map_type, zipfiles, u_map, tree, grid):
    exp = f'{map}/{prefix}{config}/{spawn_time}/{agent}/{seed}'
    ...
    obstacle_csv_path = ''
    for file in all_files:
        coverage_csv_temp = f"{exp}/coverage_matrix_{map_type}{pipuck_string}.csv"
        obstacle_csv_temp = f"{exp}/obstacle_matrix_{map_type}{pipuck_string}.csv"
        if file.startswith("experiment"):
            coverage_csv_temp = file.split('/')[0]  + '/' + coverage_csv_temp
            obstacle_csv_temp = file.split('/')[0]  + '/' + obstacle_csv_temp
        if file == coverage_csv_temp:
            coverage_csv_path = file
        elif file == obstacle_csv_temp:
            obstacle_csv_path = file
        if coverage_csv_path != '' and obstacle_csv_path != '':
            break
        
    if coverage_csv_path == '' or obstacle_csv_path == '':
        print(f"ERROR: {coverage_csv_path} or {obstacle_csv_path} not found in", experiment_is_in_zipfile)
        return None

    coverage_data = read_matrix_data(zf, coverage_csv_path)
    obstacle_data = read_matrix_data(zf, obstacle_csv_path)
    # print("checking for", csv_path, "in", experiment_is_in_zipfile)

    precision, recall,coverage, covered_invalid_area = calculate_precision_recall_coverage(u_map, tree, str_tree_with_spawn_boxes, grid, grid_with_spawn_boxes, coverage_data, obstacle_data, spawn_time, False)
    ...
    for map, map_values in categories_and_values.items():
        ...
        for end_time in end_times:
            for spawn_time in spawn_times:
                ...
                for noise in noises:
                    n_experiments = len(comm_ranges) * len(message_loss_probabilities) * len(frontier_search_radii) * len(evaporation_times)* len(agents) * len(seeds) #  * len(max_frontier_regions) * len(max_route_lengths) 
                    run_experiments = 0
                    ...
                    for frontier_search_radius in frontier_search_radii: 
                        ...
                        for agent in agents:
                            for map_type in ['returning_','finished_exploring_', 'map_relayed_']:
                                if map_type != 'map_relayed_':
                                    continue
                                ...
                                with concurrent.futures.ProcessPoolExecutor() as executor:
                                    future_to_seed = {executor.submit(process_experiment, seed, map, prefix, config, spawn_time, agent, map_type, zipfiles, u_map, tree, grid): seed for seed in seeds}
                                ...
                                print("updating columns:" ,f'{column}_{agent}_{map_type}')
                                ...
                                new_columns[f'{column}_{agent}_{map_type}precision'] = mean_precision_seeds_all
                                new_columns[f'{column}_{agent}_{map_type}recall'] = mean_recall_seeds_all
                                new_columns[f'{column}_{agent}_{map_type}coverage'] = mean_coverage_seeds_all
                                new_columns[f'{column}_{agent}_{map_type}covered_invalid_area'] = mean_covered_invalid_area_seeds_all
