...
def plot_precision_recall_with_different_configs(usb_drive, categories_and_values):
    dir = f'{usb_drive}averaged_data/fsr/accuracy_first_map_relayed/'
    ...
    for file in os.listdir(dir):
        ...
        for i,column in enumerate(columns):
            ...
            if precision_or_recall_or_coverage == 'precision':
                ...
                for j, v in enumerate(data[column]):
                    ax[agents.index(agents_string)].text(color_index, 0.05, f'{v:.2f}', ha='center', va='bottom', fontsize=8)
                ...
                #write the value in the bar
                for j, v in enumerate(data[column]):
                    ax[agents.index(agents_string)].text(color_index + bar_width, 0.05, f'{v:.2f}', ha='center', va='bottom', fontsize=8)
        ...
        fig.legend(*zip(*unique), loc='center right', bbox_to_anchor=(0.75, 0.365), fontsize=8, ncol=len(unique))
        plt.subplots_adjust(left=0.08, bottom=0.1, right=0.96, top=0.95, hspace=0.15, wspace=0.2)
        #set plot size
        fig.set_size_inches(14, 25)
        # plt.show()
        plt.savefig(f'accuracy_plots/fsr/accuracy_fsr_{map}_noise_{noise}_spawn_time_{spawn_time}.png', dpi=300, transparent=False, bbox_inches='tight')
        print(f'accuracy_plots/fsr/accuracy_fsr_{map}_noise_{noise}_spawn_time_{spawn_time}.png')
...
plot_precision_recall_with_different_configs(usb_drive, categories_and_values)
