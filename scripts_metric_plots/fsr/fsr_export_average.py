...
def export_average_certainty_coverage_nodes_with_different_configs(usb_drive, zipfiles, categories_and_values, coverage, certainty, nodes):
    if certainty:
                            ...
                            #     for max_route_length in max_route_lengths:
                                    ...
                                    for agent in agents:
                                        ...
                                        for seed in seeds:
                                            ...
                                            if coverage:
                                                #COVERAGE
                                                disregarding_evap_csv_path = ''
                                                regular_csv_path = ''
                                                for file in all_files:
                                                    disregarding_evap_csv_temp = f"{exp}/coverage_disregarding_evaporation.csv"
                                                    regular_csv_temp = f"{exp}/coverage.csv"
                                                    if file.startswith("experiment"):
                                                        disregarding_evap_csv_temp = file.split('/')[0]  + '/' + disregarding_evap_csv_temp
                                                        regular_csv_temp = file.split('/')[0]  + '/' + regular_csv_temp
                                                    if file == disregarding_evap_csv_temp:
                                                        disregarding_evap_csv_path = file
                                                    elif file == regular_csv_temp:
                                                        regular_csv_path = file
                                                    if disregarding_evap_csv_path != '' and regular_csv_path != '':
                                                        break
                                                if disregarding_evap_csv_path == '':
                                                    print("ERROR: coverage_disregarding_evaporation.csv not found in", experiment_is_in_zipfile, "/", exp)
                                                    continue
                                                if regular_csv_path == '':
                                                    print("ERROR: coverage.csv not found in", experiment_is_in_zipfile, "/", exp)
                                                    continue
                                                with zf.open(regular_csv_path) as regular_csv_file:
                                                    with zf.open(disregarding_evap_csv_path) as disregarding_evap_csv_file:
                                                        # print("reading csv:", experiment_is_in_zipfile + '/' +csv_path)
                                                        df_coverage = pd.read_csv(regular_csv_file)
                                                        df_coverage_disregarding_evap = pd.read_csv(disregarding_evap_csv_file)
                                                        df_coverage.fillna(method='ffill', inplace=True)
                                                        df_coverage_disregarding_evap.fillna(method='ffill', inplace=True)
                                                        df = pd.DataFrame()
                                                        # average coverage of all columns starting with 'pipuck'
                                                        df['coverage_regular'] = df_coverage[[column for column in df_coverage.columns if column.startswith('pipuck')]].mean(axis=1)
                                                        df['coverage_disregarding_evap'] = df_coverage_disregarding_evap[[column for column in df_coverage_disregarding_evap.columns if column.startswith('pipuck')]].mean(axis=1)
                                                        average_coverage.append(df['coverage_regular'] + df['coverage_disregarding_evap'])
                                                        # print("added to average")
                                                        if len(df_coverage['tick']) > len(time_coverage):
                                                            time_coverage = df_coverage['tick'].to_numpy()/ticks_per_second
                                            if nodes:
                                                #NODES
                                                csv_path = ''
                                                for file in all_files:
                                            ...
                                            with zf.open(csv_path) as csv_file:
                                                ...
                                                # average traveled path of all columns starting with 'pipuck'
                                                average_distance_to_deployment_site.append(df['distance_to_deployment_site'].mean(axis=0))
                                        ...
                                        if certainty:
                                            ...
                                            data_for_map_spawn_time_noise_certainty[f'fsr_{frontier_search_radius}_{agent}_all'] = mean_certainty_seeds_all[:exp_duration_index]
                                            data_for_map_spawn_time_noise_certainty[f'fsr_{frontier_search_radius}_{agent}_free'] = mean_certainty_seeds_free[:exp_duration_index]
                                            data_for_map_spawn_time_noise_certainty[f'fsr_{frontier_search_radius}_{agent}_occupied'] = mean_certainty_seeds_occupied[:exp_duration_index]
                                        if coverage:
                                            #COVERAGE
                                            ...
                                            data_for_map_spawn_time_noise_coverage[f'fsr_{frontier_search_radius}_{agent}'] = mean_coverage_seeds[:exp_duration_index]
                                        if nodes:
                                            #NODES
                                            ...
                                            data_for_map_spawn_time_noise_nodes[f'fsr_{frontier_search_radius}_{agent}_nodes'] = mean_nodes_seeds[:exp_duration_index]
                                            data_for_map_spawn_time_noise_nodes[f'fsr_{frontier_search_radius}_{agent}_leaves'] = mean_leaves_seeds[:exp_duration_index]
                                        ...
                                        data_for_map_spawn_time_noise_traveled_path[f'fsr_{frontier_search_radius}_{agent}'] = mean_nodes_seeds
                                        ...
                                        data_for_map_spawn_time_noise_battery_usage[f'fsr_{frontier_search_radius}_{agent}'] = mean_nodes_seeds
                                        #MISSION TIME
                                        average_mission_time_df = pd.DataFrame(average_mission_time)
                                        mean_nodes_seeds = average_mission_time_df.mean(axis=0).to_numpy()
                                        data_for_map_spawn_time_noise_mission_time[f'fsr_{frontier_search_radius}_{agent}'] = mean_nodes_seeds
                                        #DISTANCE TO DEPLOYMENT SITE
                                        ...
                                        data_for_map_spawn_time_noise_distance_to_deployment_site[f'fsr_{frontier_search_radius}_{agent}'] = mean_nodes_seeds
                                        #COLLISIONS
                                        ...
                                        data_for_map_spawn_time_noise_collisions[f'fsr_{frontier_search_radius}_{agent}_obstacle'] = mean_nodes_seeds
                                        ...
                                        data_for_map_spawn_time_noise_collisions[f'fsr_{frontier_search_radius}_{agent}_agent'] = mean_nodes_seeds
...
usb_drive = '/media/hugo/Thesis_Data/Base/'
zipfiles = []
completed_per_zip = []
...
export_average_certainty_coverage_nodes_with_different_configs(usb_drive, zipfiles, categories_and_values, True, False, False)
