...
def plot_traveled_path_with_different_configs(usb_drive, categories_and_values):
    dir = f'{usb_drive}averaged_data/fsr/traveled_path/'
    ...
    for file in os.listdir(dir):
        ...
        for i,column in enumerate(columns):
            ...
            #make sure the color is the same for all three certainty types
            #also make sure same config has same color, between the different agents
            if 'config_to_color_indexa' not in locals():
                config_to_color_indexa = {}
                current_color_index = 0

            if config not in config_to_color_indexa:
                config_to_color_indexa[config] = current_color_index
                current_color_index += 1
            
            color_index = config_to_color_indexa[config]
        ...
        for n_agents_string in agents:
            ...
            min_traveled_path_color_index = config_to_color_indexa[min_traveled_path_column]
            max_traveled_path_color_index = config_to_color_indexa[max_traveled_path_column]
        ...
        # plt.show()
        plt.savefig(f'traveled_path/fsr/traveled_path_{env_map}_noise_{noise}_spawn_time_{spawn_time}.png', dpi=300, transparent=False, bbox_inches='tight')
        print(f'traveled_path/fsr/traveled_path_{env_map}_noise_{noise}_spawn_time_{spawn_time}.png')
        #export counts to txt file, table format
        with open(f'traveled_path/fsr/traveled_path_{env_map}_noise_{noise}_spawn_time_{spawn_time}.txt', 'w') as f:
...
def plot_battery_usage_with_different_configs(usb_drive, categories_and_values):
    dir = f'{usb_drive}averaged_data/fsr/battery_usage/'
    if not os.path.exists(dir):
        os.makedirs(dir)
    #for all csv files in the directory
    for file in os.listdir(dir):
        ...
        for n_agents_string in agents:
            ...
            for i,column in enumerate(columns):
                ...
                if battery < min_battery_path:
                    min_battery_path = battery
                    min_battery_column = config

            
            min_battery_color_index = config_to_color_indexa[min_battery_column]
            max_battery_color_index = config_to_color_indexa[max_battery_column]
            ax[agents.index(n_agents_string)].set_ylim(total_min * 0.9, total_max * 1.1)
            ax[agents.index(n_agents_string)].bar(min_battery_color_index, min_battery_path, facecolor='none', edgecolor='springgreen', linewidth=4)
        ...
        fig.text(0.04, 0.5, 'Battery usage (mAh)', va='center', rotation='vertical')
        plt.xticks(x, config_labels, rotation=45)
        ...
        fig.set_size_inches(14, 25)
        # plt.show()
        plt.savefig(f'battery_usage/fsr/battery_usage_{env_map}_noise_{noise}_spawn_time_{spawn_time}.png', dpi=300, transparent=False, bbox_inches='tight')
        print(f'battery_usage/fsr/battery_usage_{env_map}_noise_{noise}_spawn_time_{spawn_time}.png')
        #export counts to txt file, table format
        with open(f'battery_usage/fsr/battery_usage_{env_map}_noise_{noise}_spawn_time_{spawn_time}.txt', 'w') as f:
            f.write(f'Counts for map {env_map}, noise {noise}, spawn time {spawn_time}\n')
            f.write('Counts for max ACP\n')
            f.write(tabulate(counts['fsr']['max'].items(), headers=['FSR', 'Count'], tablefmt='grid'))
            f.write('\n')
...
if not os.path.exists('traveled_path/fsr'):
    os.makedirs('traveled_path/fsr')

if not os.path.exists('battery_usage'):
    os.makedirs('battery_usage')
if not os.path.exists('battery_usage/fsr'):
    os.makedirs('battery_usage/fsr')
