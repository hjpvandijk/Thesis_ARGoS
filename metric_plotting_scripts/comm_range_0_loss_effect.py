        ...
        for end_time in end_times:
            for spawn_time in spawn_times:
                for noise in noises:
                    ...
                    fig.tight_layout()
                    for message_loss_probability in message_loss_probabilities:
                        if message_loss_probability != 0.0:
                            continue
                        n_colors = len(comm_ranges)
                        colors = plt.cm.viridis(np.linspace(0, 1, n_colors))
                        for comm_range in comm_ranges:
                            color = colors[comm_ranges.index(comm_range)]
                            for frontier_search_radius in frontier_search_radii: # is constant
                                for evaporation_time in evaporation_times: #is constant
                                    for max_frontier_cell in max_frontier_cells: # is constant
                                        for max_route_length in max_route_lengths: # is constant
                                            for agent in agents:
                                                average_all = []
                                                average_free = []
                                                average_occupied = []
                                                ...
                                                ax1[agent_index].plot(time_certainty, s_all, label=lbl, color=color, linestyle=agent_linestyles[agents.index(agent)])
                        ...
                        fig.suptitle(f'Certainty for map {map},  noise {noise}, spawn time {spawn_time}, and message loss {message_loss_probability}', wrap=True)
                        ...
                        # plt.ylim(0.05, 0.20)
                        plt.tight_layout()
                        plt.subplots_adjust(left=0.08, bottom=0.07, right=0.96, top=0.95, hspace=0.13, wspace=0.2)
                        plot_time = pd.Timestamp.now()
                        print(f"plotting took: {plot_time - start_time}")
                        plt.show()
