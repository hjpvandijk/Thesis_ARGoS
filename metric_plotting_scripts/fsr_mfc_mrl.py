...
def check_if_all_required_experiments_done(completed_experiments, categories_and_values):
    ...
    for map, map_values in categories_and_values.items():
        ...
        print("max_frontier_cells:", max_frontier_cells)
...
def plot_certainty_with_different_configs(usb_drive, zipfiles, categories_and_values):
    dir = f'{usb_drive}averaged_data/fsr_mfc_mrl/certainty/'
    ...
    for map, map_values in categories_and_values.items():
        ...
        for end_time in end_times:
            for spawn_time in spawn_times:
                for noise in noises:
                    datafile = f'{dir}{map}_{spawn_time}_noise_{noise}.csv'
                    if os.path.exists(datafile):
                        continue
                    data_for_map_spawn_time_noise = pd.DataFrame()
                    ...
                    fig.tight_layout()
                    n_colors = len(frontier_search_radii) * len(max_frontier_cells) * len(max_route_lengths)
                    ...
                    for frontier_search_radius in frontier_search_radii: 
                            for max_frontier_cell in max_frontier_cells:
                                for max_route_length in max_route_lengths:
                                    color = colors[frontier_search_radii.index(frontier_search_radius) * len(max_frontier_cells) * len(max_route_lengths) + max_frontier_cells.index(max_frontier_cell) * len(max_route_lengths) + max_route_lengths.index(max_route_length)]
                                    for agent in agents:
                                        average_all = []
                                        average_free = []
                                        average_occupied = []
                                        ...
                                        lbl = f'fsr {frontier_search_radius}, mfc {max_frontier_cell}, mrl {max_route_length}'
                                        agent_index = agents.index(agent)
                                        ax1[agent_index].plot(time_certainty, s_all, label=lbl, color=color)
                                        exp_duration_index = time_certainty.tolist().index(end_time)+1
                                        data_for_map_spawn_time_noise['time'] = time_certainty[:exp_duration_index]
                                        data_for_map_spawn_time_noise[f'fsr_{frontier_search_radius}_mfc_{max_frontier_cell}_mrl_{max_route_length}_{agent}'] = s_all[:exp_duration_index]

                                        
                    #export data to csv
                    data_for_map_spawn_time_noise.to_csv(datafile)
                    # # After all agents are plotted:
                    # lined = {}
                    # for i, agent in enumerate(agents):
                        # ax1[agent_index].set_xlabel('')
                        # ax1[agent_index].set_ylabel('Certainty', color=color)
                        # ax1[agent_index].tick_params(axis='y', labelcolor=color)
                        # ax1[agent_index].set_title(f'{n_agents} agents', wrap=True)
                        # ax1[agent_index].set_xlim(0, 400)
                        # ax1[agent_index].set_ylim(0.05, 0.25)
                    #     handles, labels = ax1[i].get_legend_handles_labels()  # Get legend items from first subplot
                    #     fig_legend = ax1[i].legend(handles, labels, loc="upper left", ncol=len(labels), fontsize=8)

                    #     # Dictionary to store line visibility

                    #     # Make legend items pickable
                    #     for legline, origline in zip(fig_legend.get_lines(), handles):
                    #         legline.set_picker(5)  # Enable clicking
                    #         lined[legline] = origline  # Map legend item to actual line
                    ...
                    #         origline.set_visible(visible)  # Toggle visibility
                    #         legline.set_alpha(1.0 if visible else 0.2)  # Dim legend entry
                    #         fig.canvas.draw_idle()  # Refresh plot

                    # # Connect event handler **only once**
                    # fig.canvas.mpl_connect("pick_event", on_pick)
                    # fig.suptitle(f'Certainty for map {map},  noise {noise}, spawn time {spawn_time}, comm range {comm_range}, message loss {message_loss_probability}, and evaporation time {evaporation_time}', wrap=True)
                    # fig.text(0.5, 0.04, 'Time (s)', ha='center')
                    # # fig.legend(loc='center left', fontsize=8)
...
def export_average_certainty_with_different_configs(usb_drive, zipfiles, categories_and_values):
    dir = f'{usb_drive}averaged_data/fsr_mfc_mrl/certainty/'
    if not os.path.exists(dir):
        os.makedirs(dir)
...
for file in os.listdir('implementation_and_examples/agent_implementation/configs/fsr_mfc_mrl'):
